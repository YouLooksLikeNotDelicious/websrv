...
int slog_open(const char *dir, const char *name, enum slog_level level, uint64_t rotate_size)
{
    if (_slog_mkdir_recursive(dir) == -1)
        return -1;
    char *filename = (char *)calloc(1, strlen(dir) + strlen(name) + 2);
    ...
    char *lockname = (char *)calloc(1, strlen(dir) + strlen(name) + 8);
    ...
    logger = (struct slog *)calloc(1, sizeof(*logger));
    logger->fd = fd;
...
static void _slog_check_file(struct timeval *tv, struct tm *tm)
{
    ...
    if (ret == 0 && st.st_size >= (off_t)logger->rotate_size) {
...
void slog_write(enum slog_level level, const char *file, int line, const char *function, const char *fmt, ...)
{
    ...
    if (header_len >= (int)sizeof(header)) { /* truncated */
    ...
    va_end(args);
    if (body_len >= (int)sizeof(body)) { /* truncated */
