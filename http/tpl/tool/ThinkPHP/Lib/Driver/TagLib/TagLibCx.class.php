...
class TagLibCx extends TagLib {
    ...
    protected $tags   =  array(
        ...
        'php'       =>  array(),
        'volist'    =>  array('attr'=>'name,id,offset,length,key,mod,listtype','level'=>3,'alias'=>'iterate'),
        'foreach'   =>  array('attr'=>'name,item,key,listtype','level'=>3),
    ...
    public function _php($attr,$content) {
        ...
        $parseStr = '|quote|; '.$content.' tpl+=|quote|';
        return $parseStr;
    }
    ...
    public function _volist($attr,$content) {
        ...
        $name  =    $tag['name'];
        $id    =    $tag['id'];
        $length = isset($tag['length'])?$tag['length']:'_list_length';
        $offset = isset($tag['offset'])?$tag['offset']:'0';
        $listtype    =    trim($tag['listtype']);
        $listtype    =    str_replace('&lt;', '<', $listtype);
        $listtype    =    str_replace('&gt;', '>', $listtype);
        $item_type_reg = '/(list|vector)<(.+)>/';
        preg_match($item_type_reg, $listtype, $matches);
        $itemtype = $matches[2];
        $mem_op = substr($listtype, -1) == '*' ? '->' : '.';
        ...
        $parseStr .= '|brace1|';
        if(0===strpos($name,':')) {
            $parseStr   .= $listtype.'__result='.substr($name,1).';';
            $name   = '__result';
        }else{
            $name   = $this->autoBuildVar($name);
        }
        $name = str_replace('"', '|quote|', $name);
        $parseStr .=  $itemtype.' _'.$id.';';
        $parseStr .=  'int _offset = '.$offset.';';
        $parseStr .=  'int _list_length = '.$name.$mem_op.'size();';
        $parseStr .=  'int _length = '.$length.';';
        $parseStr .=  'int _mod = 0;';
        $parseStr .= 'for (int _'.$key.' = _offset; _'.$key.' < _length && _'.$key.' < _list_length; _'.$key.'++)';
        $parseStr .= '|brace1|';
        $parseStr .= '_'.$id.' = '.$name.'[_'.$key.'];';
        $parseStr .= '_mod = (_'.$key.'% '.$mod.' );';
        $parseStr .= 'tpl+=|quote|'.$this->tpl->parse($content).'|quote|;';
        $parseStr .= '|brace2|';
        $parseStr .= '|brace2|';
        $parseStr .= 'tpl+=|quote|';
    ...
    public function _foreach($attr,$content) {
        ...
        $itemtype = $matches[2];
        $itemop = '';
        if (substr($itemtype, -1) == '&')
        {
            $itemtype = substr($itemtype, 0, strlen($itemtype) - 1).' *';
            $itemop = '&';
        }
        $mem_op = substr($listtype, -1) == '*' ? '->' : '.';
        $name       =   $this->autoBuildVar($name);
        $parseStr   =   '|quote|;';
        $parseStr   .=   '|brace1|';
        $parseStr .=  $itemtype.' _'.$item.';';
        $parseStr .=  'int _list_length = '.$name.$mem_op.'size();';
        $parseStr   .=   'for (int _'.$key.'=0; _'.$key.' < _list_length; _'.$key.'++)';
        $parseStr   .=   '|brace1|';
        $parseStr   .=   '_'.$item.'='.$itemop.' '.$name.'[_'.$key.'];';
        $parseStr  .=   'tpl+=|quote|'.$this->tpl->parse($content).'|quote|;';
        $parseStr   .=   '|brace2|';
        $parseStr   .=   '|brace2|';
        $parseStr   .=   'tpl+=|quote|';
    ...
    public function _if($attr,$content) {
        ...
        $parseStr  = '|quote|; ';
        $parseStr  .= '|brace1|';
        $parseStr   .=   'if('.$condition.') |brace1|'.'tpl+=|quote|'.$content.'|quote|;|brace2|';
        $parseStr  .= '|brace2|';
        $parseStr  .= 'tpl+=|quote|';
        return $parseStr;
    }
    ...
    public function _elseif($attr,$content) {
        ...
        $condition  =   $this->parseCondition($tag['condition']);
        $parseStr   =   '|quote|;|brace2| else if('.$condition.') |brace1|tpl+=|quote|';
        return $parseStr;
    }
    ...
    public function _else($attr) {
        $parseStr = '|quote|;|brace2| else |brace1|tpl+=|quote|';
        return $parseStr;
    }
