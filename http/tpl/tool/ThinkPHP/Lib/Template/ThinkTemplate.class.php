...
class  ThinkTemplate {
    ...
    public function loadTemplate ($tmplTemplateFile,$prefix='') {
        ...
        $tmplContent = preg_replace('/^(.)/m', '"$1', $tmplContent);
        $tmplContent = preg_replace('/(.)$/m', '$1"', $tmplContent);
        $tmplContent = preg_replace('/\|quote\|/', '"', $tmplContent);
        $tmplContent = preg_replace('/\|brace1\|/', '{', $tmplContent);
        $tmplContent = preg_replace('/\|brace2\|/', '}', $tmplContent);
        $tmplContent = preg_replace('/-#/', '->', $tmplContent);
        $tmplContent = $this->templateContentReplace($tmplContent);
        $tmplContent = "#define TRUE true\n#define FALSE false\nstd::string tpl; tpl.resize(4096); tpl=".$tmplContent.';';
        echo($tmplContent);
        exit();
        // 检测模板目录
    ...
    protected function parsePhp($content) {
        if(ini_get('short_open_tag')){
            ...
            $content = preg_replace('/(<\?(?!php|=|$))/i', '|quote|;tpl+= |quote|\\1|quote|;tpl+=|quote|', $content );
        }
        // PHP语法检查
    ...
    public function parseVar($varStr){
        ...
        if(!empty($varStr)){
            ...
            if('Think.' == substr($var,0,6)){
                ...
                //        $name = 'is_array(_'.$var.')?_'.$var.'[|quote|'.$vars[0].'|quote|]:_'.$var.'->'.$vars[0];
                //}
            }elseif(false !== strpos($var,'[')) {
                //支持 {$var['key']} 方式输出数组
                $name = "_".$var;
            ...
            $parseStr = '|quote|; tpl+= '.$name.'; tpl+=|quote|';
        }
        ...
        for($i=0;$i<$length ;$i++ ){
            ...
            default:  // 通用模板函数
                if(!in_array($fun,$template_deny_funs)){
                    if(isset($args[1])){
                        $args[1] = str_replace('"', '|quote|', $args[1]);
                        if(strstr($args[1],'###')){
    ...
    private function parseTemplateName($templateName){
        ...
        foreach ($array as $templateName){
            if(false === strpos($templateName,$this->config['template_suffix'])) {
                ...
                //}
                $templateName  =  $basePath.$templateName.$this->config['template_suffix'];
            }
            // 获取模板文件内容
            $parseStr .= file_get_contents($templateName);
        }
        $parseStr = '<php>{</php>' . $parseStr;
        $parseStr .= '<php>}</php>';
        return $parseStr;
    }    
}
